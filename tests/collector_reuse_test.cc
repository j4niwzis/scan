...
inline constexpr scan::fixed_string with_a_name =
    "at=(\\(([+-]?[0-9]{1,}),([+-]?[0-9]{1,})\\)) ([a-z]+)";
...
TEST(CollectorReuse, BuiltFromTheGroupsAlreadyFound) {
  ...
  const auto found = scan::match<spelled_out>.into(scan::as<point>())(text);
  ASSERT_TRUE(static_cast<bool>(found));
  EXPECT_EQ(found.get<1>().x, 3);
  EXPECT_EQ(found.get<1>().y, -4);
}

TEST(CollectorReuse, TheTypeTakesTheGroupsItSpelledOut) {
  // Two of the three groups are the type's own places, so one collector reads
  // all three of them -- and a group written after them is the next
  // collector's, without anybody counting them out by hand.
  const std::string text = "at=(3,-4) home";
  const auto found =
      scan::match<with_a_name>.into(scan::as<point>(), scan::text())(text);
  ASSERT_TRUE(static_cast<bool>(found));
  EXPECT_EQ(found.get<1>().x, 3);
  EXPECT_EQ(found.get<1>().y, -4);
  EXPECT_EQ(found.get<2>(), "home");
}
