...
Every number below was taken on one machine, and it is named because the
distances between the rows are not the same on every machine: an AMD Ryzen 9
9950X, clang 22.1.8 with libc++, `-O3 -march=native`, whole-program
optimisation. A narrower core makes the walk look worse against the machine
written out than it does here -- it executes about the same instructions and a
sixth more branches, and what that costs depends on how many a core retires in
a cycle.

The benchmarks are built with whole-program optimisation, and each engine's
code is generated by a backend run of its own. That is not tidiness: an
`-mllvm` option handed to a link is handed to one code generation, and under
LTO that code generation is every engine at once. This library is built with
`-jump-threading-across-loop-headers`, which is worth two to four per cent to a
walk written out with a label for every state -- and nineteen to the scanner
re2c generates, and minus twenty-two to CTRE. Left to reach all of them it
reverses which of the two is in front on the short records, mostly by making
the other one worse.

So a benchmark is two translation units of one file: this library's rows and
theirs, each compiling only its own functions and importing only its own
modules. Theirs are taken through a relocatable link first, which runs the
backend over them and emits machine code, and the option on the final link
cannot reach inside an object that is already machine code. Both halves get
whole-program optimisation; only this one gets the option. RE2's own library is
not covered by that and the option reaches it -- about one per cent, measured,
and said here rather than left to be found.
...
the subject, the reading is fourteen per cent ahead of the generated scanner
and a third ahead of the backtracking matcher; told only where the subject
ends, it is nineteen per cent behind the generated scanner and still eight
ahead of CTRE. Thirty characters is five fields of five letters, and a field of
five letters is a run too short to step over -- what is left is the per-field
work, and on a subject with no terminator most of that is the guard every
character read has to carry.
...
| | a pass |
| --- | --- |
| `scan::match<p>.sentinel().scalar()` | 436 ns |
| `scan::match<p>.scalar()` | 545 ns |
| re2c | 1186 ns |
| RE2 | 2616 ns |
| CTRE | 14339 ns |
...
them. Asked for a character at a time it is two and a half times the speed of the
generated scanner with a terminator, and more than twice without one. The
...
recognising the address is 2.6 nanoseconds a character, taking five fields out
of a record is 16.7. Six times, for the same engine on the same machine, and
...
| | a pass |
| --- | --- |
| making the strings, nothing scanned | 1090 ns |
| `scan::scan<f>` into strings | 2865 ns |
...
| five words into strings | 10155 ns | 3169 ns |
...
others cannot, rather than describing it. The subject is `value=(`, then heaps
of `_X` and `__XX`, then `)` and a tail of letters. A heap is a number written
as weighed marks -- the underscores say which decimal place it is, the marks
after them say how many, `X` one and `Y` two, so twelve is `__X_XX` and `__X_Y`
as well -- and the type that adds them up never sees a character of the
subject:
...
struct scan::scanner<tally> {
  ...
  static constexpr void closed_group(state_type& one, scan::group_at<0>) {
    unsigned long weight = 1;                         // a heap ends: weigh it
  ...
  static constexpr void push_group(state_type& one, scan::group_at<2>,
                                   char letter) {
    one.marks += letter == 'Y' ? 2u : 1u;             // a mark: X one, Y two
  }
...
underscores, which give the decimal place; group two is the marks, added up as
they arrive; group zero is the heap, and closing it puts the sum in its place.
That mid-parse view of the groups is the whole point, and it is what the
columns beside it have to be compared against.

Three of them: this library's own automaton written out as labels and direct
jumps -- taken off it state for state, including the two that differ only in
whether the heap has just begun, and including its register commands as named
locals, which is what a generator emits for them -- calling this scanner's own
hooks so that nothing but the way through the machine differs; the same reading
written by hand as loops and a pointer; and the library left to choose its own
walk. Ten, a hundred and a thousand heaps to a subject:
...
Which is straight lines with different intercepts, and that is the whole of it.
Fitted through the three points, a heap costs 3.19 ns here, 3.14 ns written out
and 3.78 ns by hand; entering costs about 120 ns here against 27 written out
and 14 by hand. Per heap the walk and its own machine written out are the same
within two per cent -- which is what it should be, since they are the same
machine -- and the walk is sixteen per cent cheaper than the reading written by
hand. What it is not is cheap to enter, and on ten heaps that is the whole of
the column: three times behind on a subject of forty-six characters, level on
one of three thousand, ahead of the hand at both ends of that.

The hundred and twenty nanoseconds are the walk arranging what it needs before
it reads a character: the gathering the fold is kept in, the marks the machine
writes, and the commands it runs before the first character. A reading that is
handed a line at a time pays it once a line, and that is where it is worth
taking apart next; a reading handed a file does not notice.
...
wrong: 11.9 ns a heap against 3.19, nearly four times. Asked for `.scalar()` it
is level with its own machine written out. Two benchmarks now say the same
thing about that threshold, which is where to look next.
