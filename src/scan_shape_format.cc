// Reading a format against the type it is scanned into.
//
// A place standing for a leaf becomes that leaf's own pattern; a place standing
// for a type that says a format of its own becomes that format, read against
// that type. What comes out is the one pattern the automaton is built from, the
// parameters each place was written with, and how many turns each place may
// take. Below that is the glue: a scanner asked for a value, for a state, for
// its groups -- in the shapes it may have been written in.

export module scan.shape.format;
...
export inline constexpr std::size_t turns_unbounded = ~std::size_t{0};
...
export constexpr void say_raw_end(spread_format& made) { made.text.push_back(')'); }
...
export constexpr void say_written_pattern(spread_format& made, std::string_view text) {
...
export constexpr void copy_until_place(spread_format& made, std::string_view text,
                                std::size_t& position) {
...
export [[nodiscard]] constexpr std::array<std::pair<std::size_t, std::size_t>, 16>
...
export template <class Type, bool Within>
...
export template <class Type, std::size_t Extent, std::size_t... Index>
...
export template <class Type, class FailureType, bool ToldApart,
...
export template <class Type, std::size_t Branch>
...
export template <class Root, fixed_string Format>
...
// call it named.
export template <class FailureType, class... Parts>
...
export template <class Type, fixed_string Format, fixed_string Opening,
...
export template <class Type, fixed_string Format, bool AbsentIsEmpty = false>
...
export template <class Type, fixed_string Format, int Sentinel = -1,
...
export template <class Type, bool AsOutput = false>
...
// before it.
export template <class Type, std::size_t Group>
